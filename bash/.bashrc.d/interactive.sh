  ...
  __print_status() {
    ...
    then
      ...
      then
        # jujutsu(colocate)
        [ -n "$is_jujutsu" ] &&
          prompt="${prompt} ${cyan}(${repo_type}/${red}jujutsu${reset}${cyan}:${reset}" ||
          prompt="${prompt} ${cyan}(${repo_type}:${reset}"
      else
        # git-submodule or git-worktree
    ...
    then
      prompt="${prompt} ${cyan}(jujutsu)${reset}"
    fi
...
# atuin {{{
# NOTE: must be in global scope; bash-preexec's precmd_functions/preexec_functions
# become local and are lost if sourced inside a function
if type atuin >/dev/null 2>&1
then
  for __bash_preexec in \
    "$HOME/.nix-profile/share/bash/bash-preexec.sh" \
    "$HOME/.local/state/nix/profiles/profile/share/bash/bash-preexec.sh" \
    "/etc/profiles/per-user/$USER/share/bash/bash-preexec.sh"
  do
    # shellcheck disable=SC1090
