...
__main() {
  ...
  local -r sh_path="$(readlink -f "$HOME/.bashrc.d/path.sh")"
  local -r sh_user_path="$HOME/.cache/sh_user_path"
  if [ -r "$sh_user_path" ] && [ "$sh_user_path" -nt "$sh_path" ]
  then
    # shellcheck disable=SC1090
    source "$sh_user_path"
  else
    # shellcheck disable=SC1090
    [ -r "$sh_path" ] && source "$sh_path"
  fi
  ...
  # shellcheck disable=SC1090
  [ -r "$ssh_agent_info" ] && source "$ssh_agent_info"

  command ssh-add -l >/dev/null 2>&1
  local -r ssh_add_status=$?

  # ssh-add is unable to contact the authentication agent
  if [ "$ssh_add_status" -eq 2 ]
  then
    # force remove
